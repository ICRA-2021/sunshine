...
namespace sunshine {
    ...
    static inline cell_pose_t toCellId(word_pose_t const &word, std::array<double, POSEDIM> cell_size) {
        return {static_cast<CellDimType>(word[0] / cell_size[0]), static_cast<CellDimType>(word[1] / cell_size[1]),
              static_cast<CellDimType>(word[2] / cell_size[2]), static_cast<CellDimType>(word[3] / cell_size[3])};
    }
