...
private:
    bool isValidPosition() const
    {
        ...
        return lx >= 0 && (lx + scaled_width) < base_image.cols && ly >= 0 && (ly + scaled_height) < base_image.rows;
    }
...
template <typename DepthType = double>
sensor_msgs::PointCloud2Ptr getFlatPointCloud(cv::Mat const& rgbImage, double const width, double const height, DepthType const z, std_msgs::Header const& header = {})
{
    ...
    for (auto row = 0; row < rgbImage.rows; row++) {
        for (auto col = 0; col < rgbImage.cols; col++) {
            pcIterator->data.x = static_cast<float>((width * col) / rgbImage.cols);
            pcIterator->data.y = static_cast<float>((height * row) / rgbImage.rows);
