...
struct Observation
{
  ...
  Observation(decltype(Observation::frame) frame, decltype(Observation::timestamp) timestamp, decltype(Observation::id) id)
          : frame(std::move(frame)), timestamp(timestamp), id(id) {}
...
struct ImageObservation : public Observation
{
  ...
  ImageObservation(decltype(Observation::frame) frame,
                   ...
                   cv::Mat image)
          : Observation(std::move(frame), timestamp, id), image(std::move(image)) {}
...
struct SemanticObservation : public Observation
{
  ...
  SemanticObservation(decltype(Observation::frame) const &frame,
          ...
          : Observation(frame, timestamp, id), observations(observations), observation_poses(observationPoses) {}
  ...
  SemanticObservation(decltype(Observation::frame) const &frame,
          ...
          : Observation(frame, timestamp, id), observations(std::move(observations)), observation_poses(std::move(observationPoses)) {}
...
struct Segmentation : public SemanticObservation<label_type, pose_dim, cell_pose_type>
{
  ...
  Segmentation(decltype(Observation::frame) const &frame,
          ...
          : SemanticObservation<label_type, pose_dim, cell_pose_type>(frame, timestamp, id, labels, poses), cell_size(cell_size) {}
