...
cos_distances = np.zeros((n_samples, n_samples))
for row in trange(n_samples):
    for col in range(row + 1, n_samples):
        ...
        cos_distances[row][col] = distributions[row,:].dot(distributions[col,:])
similarity = 1 - distances/2.
cos_similarity = 1 - cos_distances
data = cos_similarity[np.triu_indices(n_samples,1)]
