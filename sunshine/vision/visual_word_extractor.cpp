...
//    double const real_inv_width = 1. / img.cols, real_inv_height = 1. / img.rows;
        ...
        sz->seq = (sunshine::seq_duration == 0)
                  ? msg->header.seq
                  : static_cast<uint32_t>((msg->header.stamp.sec - sunshine::seq_start + msg->header.stamp.nsec / 1E9)
                ...
                if (transform_recvd) {
                    ...
                    try {
                        ...
                        transform_msg = tf_buffer->lookupTransform(world_frame_name,
                                                                   sensor_frame_name,
                                                                   msg->header.stamp,
                                                                   ros::Duration(0.5 / sunshine::rate));
...
int main(int argc, char **argv) {
    ...
    std::string vocabulary_filename, texton_vocab_filename, image_topic_name, feature_descriptor_name, pc_topic_name, transform_topic_name, world_frame_name, sensor_frame_name;
    ...
    std::string const bagfile = nhp.param<string>("image_bag", "");
