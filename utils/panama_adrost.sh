...
tmux new-session -d -s foo 'roslaunch sunshine panama.launch image_topic:=/bag3/camera/image_raw basename:=rost_1 || bash'
tmux rename-window 'panama_adrost'
tmux select-window -t foo:0
tmux split-window -h 'roslaunch sunshine panama.launch image_topic:=/bag18/camera/image_raw basename:=rost_2 || bash'
